...
echo 'Forcing 2D...'
time psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/force2D.sql
echo 'Setting SRID, indexing, and vacuuming facilities and dcp_mappluto...'
time psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/setSRID_4326.sql
time psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/vacuum.sql
echo 'Spatial join with boroughs...'
time psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/join_boro_pregeoclient.sql
...
time psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/join_boro.sql
time psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/clean_invalidBIN.sql
time psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/copy_backup1.sql
...
time psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/join_PLUTOtabular.sql

## SPATIAL JOINS WITH PLUTO TO GET BBL AND OTHER MISSING INFO

echo 'Spatially joining with dcp_mappluto...'
time psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/join_PLUTOspatial.sql
echo 'Done spatially joining with dcp_mappluto'
time psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/vacuum.sql
time psql -U $DBUSER -d $DBNAME -f ./2_assembly/standardize/standardize_address.sql
...
time psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/bbl2bin.sql
echo 'Creating a backup copy before overwriting any geometries...'
time psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/copy_backup2.sql
...
time psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/bin2overwritegeom.sql
echo 'Done overwriting geometry using BIN centroid'

echo 'Overwriting geometry using BBL centroid...'
time psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/bbl2overwritegeom.sql
...
time psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/calcxy.sql
...
time psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/proptype_plazas.sql
time psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/vacuum.sql

## Create backup table before merging and dropping duplicates
echo 'Creating backup before merging and dropping duplicates...'
time psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/copy_backup3.sql