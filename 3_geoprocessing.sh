...
# psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/copy_backup0.sql
echo 'Forcing 2D...'
...
## This joins borough onto records that do not have borough or zip code already assigned.
## This distinction is important because some records came with coordinates that were the result of incorrect geocoding.
## There are records that have addresses that exist in multiple boroughs, and there are cases where a record comes with 
## boro=Manhattan but it's coordinates are at the location at the same address that's in Brooklyn instead. 
## We want to be careful to not override the borough values that were provided and should be skeptical of coordinates 
## provided on the open data portal that were likely generated automatically without any QA/QC. 
## We only want to fill in borough for the cases where no other borough or zipcode value was provided so we enable 
## potential for finding matches in GeoClient what wouldn't be possible otherwise.
...
psql -U $DBUSER -d $DBNAME -f ./3_geoprocessing/copy_backup.sql