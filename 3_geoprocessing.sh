...
# time psql -d facdb -U dbadmin -f ./3_geoprocessing/copy_backup0.sql
echo 'Forcing 2D...'
time psql -d facdb -U dbadmin -f ./3_geoprocessing/force2D.sql
echo 'Setting SRID, indexing, and vacuuming facilities and dcp_mappluto...'
time psql -d facdb -U dbadmin -f ./3_geoprocessing/setSRID_4326.sql
time psql -d facdb -U dbadmin -f ./3_geoprocessing/vacuum.sql
echo 'Spatial join with boroughs...'
time psql -d facdb -U dbadmin -f ./3_geoprocessing/join_boro_pregeoclient.sql
