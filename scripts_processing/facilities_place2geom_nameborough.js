...
var nullGeomQuery = 'SELECT DISTINCT borough, facilityname FROM facilities WHERE geom IS NULL AND facilityname IS NOT NULL AND borough IS NOT NULL';
...
var geoclientTemplate1 = 'https://api.cityofnewyork.us/geoclient/v1/place.json?name={{facilityname}}&borough={{borough}}&app_id={{app_id}}&app_key={{app_key}}';
...
function addressLookup1(row) {
  console.log('Looking up place', row.borough.trim(), row.facilityname.trim())

      var apiCall1 = Mustache.render(geoclientTemplate1, {
        
        // MAKE SURE THESE MATCH THE FIELD NAMES
        facilityname: row.facilityname.replace("/", "").replace("\"", "").replace("!", "").split(', Inc')[0].split(' Inc')[0].split(' Ltd')[0].split(', Ltd')[0].trim(),
...
function updateFacilities(data, row) {
  ...
  var insertTemplate = 'UPDATE facilities SET geom=ST_SetSRID(ST_GeomFromText(\'POINT({{longitude}} {{latitude}})\'),4326), latitude=\'{{latitude}}\', longitude=\'{{longitude}}\', bbl=ARRAY[\'{{bbl}}\'], bin=ARRAY[\'{{bin}}\'], zipcode=\'{{zipcode}}\', city=initcap(\'{{city}}\'), processingflag=\'place2geom_nameborough\' WHERE (facilityname=\'{{facilityname}}\' AND geom IS NULL'
